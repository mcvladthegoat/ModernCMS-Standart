<?php	
	...
	<br><br><br>
	<p class=header>VK Parser</p><h3><a href=".SERVERADDR."/moderncms/main.php>Go Home</a></h3>
	<div id=lefttab>
	...
	$vk_get = mysql_query("SELECT * from vkparser WHERE website = '$serveraddr_var'", $link);
	if(mysql_num_rows($vk_get)){
		while($out = mysql_fetch_array($vk_get)){
			echo $i.". ".$out["group_name"]."&nbsp&nbsptransfers to <b>".$out["category_select"]."</b>&nbsp&nbsp&nbsp<a href=index_2.php?func=group_del&group_name=".$out["group_name"]."><i><u>Delete this select</u></i></a>&nbsp&nbsp<a href=index_3.php?group_name=".$out["group_name"]."><i><b>Upload all pages</b></i></a>";
			$i++;
			echo "<br>";
		}
	}
	echo "<br>";
	echo SERVERADDR;
	echo "<p>Set up NEW connection with <a href=http://m.vk.com>m.vk.com</a>:</p>";
	echo "<form action=index_2.php?func=group_create method=POST>
			<p>Address in vk network-><input type=text name=group_name></p>
